...
netcup VPS Lite 2: 4 vCore, 8 GB, 157 GB disk, Debian 13, EUR 8.11/month.
...
3. Write `.env` from `.env.example`. On a public deployment the values that
   ...
   MCP_ALLOWED_HOSTS=api.159-195-250-205.sslip.io
   ```

   `MCP_ALLOWED_HOSTS` is the one setting whose absence cannot be caught
   locally. The MCP SDK arms an anti DNS-rebinding allowlist of localhost,
   Caddy forwards the public `Host` unchanged, and the result is **421 on
   every MCP request** on a server whose tests all passed. Empty turns the
   check off, which is defensible behind a proxy that already routes on
   Host; naming the hostname is better.
...
5. Create the demo key and keep it:
   ...
   Create the MCP key the same way and keep it separately:
   `docker compose exec api python scripts/create_api_key.py mcp-demo 10 | tee .mcp-key`.
   It is published in the README, so it must be revocable on its own: an
   abuse arriving through `/mcp` must not consume the quota the Streamlit
   demo runs on. The quota is in audio minutes and none of the three MCP
   tools transcribes, so it bounds nothing those tools do - it is there so
   that a key printed in a public repo cannot transcribe without limit.
6. Restore the corpus, after the API has applied its migrations:
   ...
   It refuses to apply twice. The file is generated by `scripts/dump_seed.sh`
...
8. Check `/mcp` **through the proxy**, not against the local port, which is
   where a Host allowlist failure hides:
   `curl -s -o /dev/null -w '%{http_code}\n' -X POST https://api.<...>.sslip.io/mcp`
   must print `401`, not `421`, and the same call with
   `-H "X-API-Key: $(cat .mcp-key)"` and an MCP body must answer with the
   three tools.
...
write `<<'SQL' ... SQL < /dev/null`, because the later redirect wins and psql reads
nothing.
